...
export default function PartsSellerForm() {
  const { t } = useLanguage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API}/register/parts-seller`, {
        method: "POST",
  ...
  if (success) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted">{t("successMessageSeller")}</p>
  ...
  const inputClass = "w-full px-4 py-3 bg-input-bg border border-input-border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 placeholder-muted-foreground transition-all text-foreground";
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addWorkplace} className="flex items-center gap-1 text-sm text-purple-500 hover:text-purple-400 transition-colors">
        ...
        {workplaces.map((workplace, index) => (
          <div key={index} className="p-5 bg-input-bg/50 border border-input-border/50 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input type="text" value={workplace.name} onChange={(e) => updateWorkplace(index, "name", e.target.value)} required placeholder={t("workplaceNamePlaceholderSeller")} className={inputClass} />
      ...
      <button type="submit" disabled={loading} className="w-full py-3.5 bg-gradient-to-r from-purple-500 to-violet-600 rounded-xl font-semibold text-white hover:from-purple-600 hover:to-violet-700 transition-all shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40 disabled:opacity-50 disabled:cursor-not-allowed">
        {loading ? (
