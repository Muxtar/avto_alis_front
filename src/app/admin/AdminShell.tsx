...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { href: "/admin/identity", label: "Kimlik", icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2.5" y="4.5" width="19" height="15" rx="2.5" strokeWidth={1.5} /><circle cx="8.5" cy="11" r="2.2" strokeWidth={1.5} /><path strokeLinecap="round" strokeWidth={1.5} d="M5 16.2c.7-1.4 2-2.2 3.5-2.2s2.8.8 3.5 2.2M14.5 9.5h4.5M14.5 12.5h4.5M14.5 15.5h3" /></svg>
    )},
    ...
    { href: "/admin/ai", label: "Süni intellekt", icon: (
    ...
    { href: "/admin/support", label: "Dəstək", icon: (
    ...
    { href: "/admin/outreach", label: "Sosial mesajlar", icon: (
    ...
    { href: "/admin/payouts", label: "Satıcı ödənişləri", icon: (
    ...
    { href: "/admin/payouts/businesses", label: "Biznes hesablaşması", icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg>
    )},
    { href: "/admin/refunds", label: "Qaytarılmalı pul", icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" /></svg>
    )},
    { href: "/admin/audit", label: "Audit jurnalı", icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" /></svg>
    )},
    { href: "/admin/admins", label: "Adminlər", icon: (
  ...
  // Sidebar linkinin tələb etdiyi icazə modulu (yoxdursa — hər admin görür).
  const linkPerm: Record<string, string> = {
    "/admin/users": "users", "/admin/listings": "listings", "/admin/orders": "orders",
    "/admin/returns": "returns", "/admin/finance": "finance", "/admin/businesses": "businesses",
    "/admin/seller-applications": "kyc", "/admin/identity": "kyc", "/admin/credentials": "credentials",
  ...
  // "Adminlər" yalnız super-admində. me hələ yüklənməyibsə hamısını göstərmə
  // (yanıp-sönməsin) — me gələnə qədər yalnız dashboard.
  const visibleLinks = links.filter((l) => {
    const perm = linkPerm[l.href];
    if (!perm) return true; // /admin dashboard
    if (!me) return false;
